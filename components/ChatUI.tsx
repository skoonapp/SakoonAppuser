...
const ChatUI: React.FC<ChatUIProps> = ({ session, user, onLeave, onSessionActive }) => {
  ...
  useEffect(() => {
    ...
    const initZego = async () => {
      ...
      try {
        const kitToken = await fetchZegoToken(session.id, user.uid, user.name);
        ...
        zp.joinRoom({
          ...
          onUserJoin: (users: any[]) => {
            console.log('A user joined the chat:', users);
            setStatus('connected');
          ...
          onUserLeave: (users: any[]) => {
            console.log('A user left the chat:', users);
         ...
         console.error("Error initializing Zego UI Kit for Chat:", err);
         setErrorMessage(`चैट शुरू करने में एक तकनीकी त्रुटि हुई। (${err.message})`);
         setStatus('error');
      }
    };
    ...
    failTimeout = window.setTimeout(() => {
      ...
      if (zpInstanceRef.current === null) {
        ...
        setErrorMessage("चैट सेवा से कनेक्ट नहीं हो सका। कृपया अपना नेटवर्क जांचें।");
        setStatus('error');
      }
    }, 15000);
  ...
  }, [session.id, user.uid, user.name, onLeave, onSessionActive, retryCount]);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 text-white flex flex-col items-center justify-center p-0 md:p-4 transition-all duration-300">
      
        {/* Zego Container, takes up the whole screen. Its internal UI has a chat window. */}
        <div 
            ref={containerRef}
        ...
        {/* Custom UI Overlay (for connecting/error states) */}
        <div className={`z-20 w-full h-full flex flex-col items-center justify-center text-center transition-opacity duration-500 bg-slate-900 ${status === 'connected' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          ...
          {status === 'connecting' && (
            <>
              ...
              <h2 className="text-3xl font-bold text-white mb-2">
                {listener ? `${listener.name} के साथ चैट शुरू हो रही है...` : 'श्रोता से कनेक्ट हो रहा है...'}
              </h2>
